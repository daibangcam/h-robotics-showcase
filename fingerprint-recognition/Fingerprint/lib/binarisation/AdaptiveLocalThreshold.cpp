...
namespace localThreshold {
    ...
    void binarisation(cv::Mat& im, int numTilesWidth, int numTilesHeight){
		...
		unsigned int* thresholds=new unsigned int[numOfTiles+numTilesWidth+numTilesHeight+1];  //Randbereiche dazu addiert
		unsigned char* deltas=new unsigned char[numOfTiles+numTilesWidth+numTilesHeight+1];
        int i, j, k, l, t;
        t = 0;
        //Laplace
        /*int kernel_size = 3;
    ...
    void medianThresholding(unsigned int thresholds[], int numOfThresholds){
		unsigned int* sortedThresholds=new unsigned int[numOfThresholds];
