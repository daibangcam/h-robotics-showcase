...
double ticks = cvGetTickFrequency() * 1.0e3;
...
int LastX0, LastX1, LastX2, LastX3, LastX4;
int LastY0, LastY1, LastY2, LastY3, LastY4;

//////////////////// EMA ////////////////////

float LastX5, LastX6, LastX7, LastX8, LastX9;
float LastY5, LastY6, LastY7, LastY8, LastY9;
...
void reset_mask(IplImage* msk1, IplImage* msk2, IplImage* msk3);
...
void main()
{
	...
	unsigned short int coordinates[1000] = { 0 };
	...
	while (1)
	{
		...
		now = cvGetTickCount();
		frame_time = (double)(now - then) / ticks;
		...
		now = cvGetTickCount();
		hand_time = (double)(now - then) / ticks;
		...
		now = cvGetTickCount();
		finger_time = (double)(now - then) / ticks;
		...
		if (rs_mask == 0)
		{
			reset_mask(mask, draw_mask, symbol_mask);
			rs_mask = 1;
		}
		...
		if (num_fingers == 1 && dis < 60)
		{
			...
			if (EMA_Init <= TIME_PERIODS)
			{
				...
				EMA_Init++;
				EMA_X = (LastX0 + LastX1 + LastX2 + LastX3 + LastX4) / EMA_Init;
				EMA_Y = (LastY0 + LastY1 + LastY2 + LastY3 + LastY4) / EMA_Init;
			}
			else
			{
			...
			coordinates[num_coordinates + 1] = pt2.y;
			/*cout << "x = " << toado[n_toado] << endl;
			cout << "y = " << toado[n_toado+1] << endl;*/
			num_coordinates += 2;
		}
		...
		if (num_fingers == 5)
		{
			...
			/*for (int i = 0; i <= num_coordinates; i+=2)
			{
				draw_coordinate << coordinates[i] << "," << coordinates[i+1] << endl;
				cout << "x = " << coordinates[i] << " y = " << coordinates[i+1] << endl;
			}*/
		}
		...
		now = cvGetTickCount();
		track_time = (double)(now - then) / ticks;
