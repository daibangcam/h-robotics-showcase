...
void main()
{
	...
	unsigned short int num_draw_coordinates = 1;

	unsigned short int side_1, side_2;
	...
	while (1)
	{
		...
		if (hand_mode == 2)
		{
			...
			if (last_num_fingers == 2 && num_fingers == 3)
			{
				...
				// calculate data array to send serial
				num_data_byte = num_draw_coordinates - 1;
				draw_coordinates[0] = num_data_byte + 360;

				for (int i = 0; i <= num_data_byte; i++)
				{
					data_byte[2 * i] = draw_coordinates[i] / 256;
					data_byte[2 * i + 1] = draw_coordinates[i] % 256;
