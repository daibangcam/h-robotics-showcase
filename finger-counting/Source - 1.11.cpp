...
void main()
{
	...
	while (1)
	{
		...
		while (draw_mode)
		{
			...
			if (hand_mode == 2)
			{
				...
				if (num_fingers == 1 && distance > 0 && distance < 60)
				{
					...
					EMA_coordinate << pt2.x << "," << h - pt2.y << "," << EMA_X << "," << h - EMA_Y << endl;
		...
		while (process_mode)
		{
			...
			now1 = cvGetTickCount();
			cout << "EMA 2 = " << (now1 - then1) / ticks << endl;
			then1 = cvGetTickCount();
			...
			now1 = cvGetTickCount();
			cout << "EMA 3 = " << (now1 - then1) / ticks << endl;
			then1 = cvGetTickCount();
			...
			now1 = cvGetTickCount();
			cout << "get angles = " << (now1 - then1) / ticks << endl;
			then1 = cvGetTickCount();
			...
			now1 = cvGetTickCount();
			cout << "scale angles = " << (now1 - then1) / ticks << endl;
			then1 = cvGetTickCount();
			...
			now1 = cvGetTickCount();
			cout << "serial = " << (now1 - then1) / ticks << endl;
			then1 = cvGetTickCount();
			...
			now1 = cvGetTickCount();
			cout << "send = " << (now1 - then1) / ticks << endl;
		}
		if ((!frame) || (c == 27)) break;
	}
