...
#define no_fingers 45					//45 //58
#define threshold_fingers 20
#define threshold_blob_height 120		//120 //50
#define threshold_blob_width 160

#define thresh_1 0.075
#define thresh_2 0.45
...
void main()
{
	...
	log_coordinates << "x,y,EMA_x,EMA_y,EMA1_x,EMA1_y" << endl;
	...
	while (1)
	{
		...
		while (draw_mode)
		{
			...
			if (hand_mode == 2)
			{
				...
				if (num_fingers == 1 && distance > 0 && distance < 60)
				{
					...
					log_coordinates << backup_pt.x << "," << h - backup_pt.y << "," << pt2.x << "," << h - pt2.y << "," << EMA_X << "," << h - EMA_Y << endl;

					//log EMA coordinates
					//EMA_coordinate << backup_pt.x << "," << h - backup_pt.y << "," << EMA_X << "," << h - EMA_Y << endl;
		...
		while (process_mode)
		{
			...
			// calculate SMA for 2nd EMA filter
			float SMA_1_x = (coordinates[0] + coordinates[2] + coordinates[4] + coordinates[6] + coordinates[8]) / 5;
			float SMA_1_y = (coordinates[1] + coordinates[3] + coordinates[5] + coordinates[7] + coordinates[9]) / 5;
			coordinates_1[0] = ((coordinates[0] - SMA_1_x) * 2 / (N + 1)) + SMA_1_x;
			coordinates_1[1] = ((coordinates[1] - SMA_1_y) * 2 / (N + 1)) + SMA_1_y;
			...
			// calculate SMA for 3rd EMA filter
			float SMA_2_x = (coordinates_1[0] + coordinates_1[2] + coordinates_1[4] + coordinates_1[6] + coordinates_1[8]) / 5;
			float SMA_2_y = (coordinates_1[1] + coordinates_1[3] + coordinates_1[5] + coordinates_1[7] + coordinates_1[9]) / 5;
			coordinates_2[0] = ((coordinates_1[0] - SMA_2_x) * 2 / (N + 1)) + SMA_2_x;
			coordinates_2[1] = ((coordinates_1[1] - SMA_2_y) * 2 / (N + 1)) + SMA_2_y;
			...
			//// log 2nd EMA & 3rd EMA coordinates 
			EMA_coordinate << endl;
			...
			side_1 = round(num_coordinates*thresh_1) * 2;
			side_2 = round(num_coordinates*thresh_2) * 2;
