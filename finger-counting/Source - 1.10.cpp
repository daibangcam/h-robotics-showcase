...
#define no_fingers 58
#define threshold_fingers 20
#define threshold_blob_height 50
...
unsigned short int LastX01, LastX11, LastX21, LastX31, LastX41;
unsigned short int LastY01, LastY11, LastY21, LastY31, LastY41;

unsigned short int LastX02, LastX12, LastX22, LastX32, LastX42;
unsigned short int LastY02, LastY12, LastY22, LastY32, LastY42;
...
void update_EMA1(int x, int y);
void update_EMA2(int x, int y);
...
void main()
{
	...
	// Initial Coordinates
	float coordinates[num];
	unsigned short int num_coordinates = 0;

	float coordinates_1[num];
	float coordinates_2[num];

	unsigned short int temp, side_1, side_2;

	float delta_x, delta_y, distance;
	unsigned short int angle;

	unsigned short int draw_angles[num];
	unsigned short int num_draw_angles = 0;

	unsigned short int draw_scale_angles[num];
	...
	float E_SMA_X1, E_SMA_Y1, EMA_X1, EMA_Y1;
	unsigned char EMA_Init1 = 0;
	EMA_X1 = EMA_Y1 = 0.0;

	float E_SMA_X2, E_SMA_Y2, EMA_X2, EMA_Y2;
	unsigned char EMA_Init2 = 0;
	EMA_X2 = EMA_Y2 = 0.0;
	...
	// Log Time
	ofstream log_process;
	log_process.open("_log_processing_time.csv");
	log_process << "Frame, Hand, Finger, Track, FPS" << endl;

	// Log Coordinates
	ofstream log_coordinates;
	log_coordinates.open("_log_coordinates.csv");
	//log_coordinates << "x, y" << endl;

	// Log Draw Angles
	ofstream log_draw_angles;
	log_draw_angles.open("_log_draw_angles.csv");
	//log_draw_angles << "angle" << endl;

	// Log Draw Scale Angles
	ofstream log_draw_scale_angles;
	log_draw_scale_angles.open("_log_draw_scale_angles.csv");
	//draw_coordinate << "angle" << endl;

	// Log Serial Angles
	ofstream log_serial_angles;
	log_serial_angles.open("_log_serial_angles.csv");
	//log_serial_angles << "high byte" << "," << "low_byte" << endl;

	// Log EMA Coordinates
	ofstream EMA_coordinate;
	EMA_coordinate.open("_log_coordinates_EMA.csv");
	...
	char start_byte[] = "s";
	char end_byte[] = "e";
	unsigned char data_byte[num];
	unsigned short int num_data_byte = 2;
	...
	// Load Video
	capture = cvCaptureFromFile("input_3.mp4");
	//if (!capture)
	//{
	...
	while (1)
	{
		...
		while (draw_mode)
		{
			...
			// flip and resize frame from camera
			//cvFlip(frame, frame, 1);
			//cvResize(frame, frame_small, 1);

			//flip and resize frame from video
			cvResize(frame, frame_small, 1);
			...
			if (hand_mode == 2)
			{
				...
				if (EMA_Init1 <= N)
				{
					update_EMA1(t1_x, t1_y);
					EMA_Init1++;
					if (EMA_Init1 == N)
					{
						E_SMA_X1 = (LastX01 + LastX11 + LastX21 + LastX31 + LastX41) / N;
						E_SMA_Y1 = (LastY01 + LastY11 + LastY21 + LastY31 + LastY41) / N;
						EMA_X1 = E_SMA_X1;
						EMA_Y1 = E_SMA_Y1;
					}
				}
				...
				{
					EMA_X1 = (((float)t1_x - EMA_X1) * 2 / (N + 1)) + EMA_X1;
					EMA_Y1 = (((float)t1_y - EMA_Y1) * 2 / (N + 1)) + EMA_Y1;
					//pt2.x = EMA_X;
					//pt2.y = EMA_Y;
				}
				...
				if (EMA_Init2 <= N)
				{
					update_EMA2(t2_x, t2_y);
					EMA_Init2++;
					if (EMA_Init2 == N)
					{
						E_SMA_X2 = (LastX02 + LastX12 + LastX22 + LastX32 + LastX42) / N;
						E_SMA_Y2 = (LastY02 + LastY12 + LastY22 + LastY32 + LastY42) / N;
						EMA_X2 = E_SMA_X2;
						EMA_Y2 = E_SMA_Y2;
					}
				}
				...
				{
					EMA_X2 = (((float)t2_x - EMA_X2) * 2 / (N + 1)) + EMA_X2;
					EMA_Y2 = (((float)t2_y - EMA_Y2) * 2 / (N + 1)) + EMA_Y2;
					//pt2.x = EMA_X;
					//pt2.y = EMA_Y;
				}
				...
				{
					// log coordinates	
					log_coordinates << pt2.x << "," << h - pt2.y << endl;

					//log EMA coordinates			
					//EMA_coordinate << EMA_X << "," << h - EMA_Y << endl;
					EMA_coordinate << pt2.x << "," << h - pt2.y << "," << EMA_X << "," << h - EMA_Y << "," << EMA_X1 << "," << h - EMA_Y1 << "," << EMA_X2 << "," << h - EMA_Y2 << endl;
					...
					coordinates[num_coordinates] = EMA_X;
					coordinates[num_coordinates + 1] = h - EMA_Y;
					num_coordinates += 2;
				}
			...
			log_process << frame_time << "," << hand_time << "," << finger_time << "," << track_time << "," << 1000 / track_time << endl;
		...
		while (process_mode)
		{
			...
			float SMA_1 = (coordinates[0] + coordinates[2] + coordinates[4] + coordinates[6] + coordinates[8]) / 5;
			float SMA_2 = (coordinates[1] + coordinates[3] + coordinates[5] + coordinates[7] + coordinates[9]) / 5;
			coordinates_1[0] = ((coordinates[0] - SMA_1) * 2 / (N + 1)) + SMA_1;
			coordinates_1[1] = ((coordinates[1] - SMA_2) * 2 / (N + 1)) + SMA_2;

			for (int i = 0; i < num_coordinates; i++)
			{
				coordinates_1[i + 2] = ((coordinates[i + 2] - coordinates_1[i]) * 2 / (N + 1)) + coordinates_1[i];
				
			}
			
			float SMA_3 = (coordinates_1[0] + coordinates_1[2] + coordinates_1[4] + coordinates_1[6] + coordinates_1[8]) / 5;
			float SMA_4 = (coordinates_1[1] + coordinates_1[3] + coordinates_1[5] + coordinates_1[7] + coordinates_1[9]) / 5;
			coordinates_2[0] = ((coordinates_1[0] - SMA_3) * 2 / (N + 1)) + SMA_3;
			coordinates_2[1] = ((coordinates_1[1] - SMA_4) * 2 / (N + 1)) + SMA_4;

			for (int i = 0; i < num_coordinates; i++)
			{
				coordinates_2[i + 2] = ((coordinates_1[i + 2] - coordinates_2[i]) * 2 / (N + 1)) + coordinates_2[i];
			}

			EMA_coordinate << endl;

			for (int i = 0; i < num_coordinates / 2; i++)
			{
				EMA_coordinate << coordinates_1[2 * i] << "," << coordinates_1[2 * i + 1] << "," << coordinates_2[2 * i] << "," << coordinates_2[2 * i + 1] << endl;
			}

			//for (int i = 0; i < num_coordinates - 3; i += 2)
			side_1 = round(num_coordinates*0.05) * 2;
			side_2 = round(num_coordinates*0.45) * 2;

			//cout << "side_1 = " << side_1 << endl;
			//cout << "side_2 = " << side_1 << endl;

			for (int i = side_1; i < side_2; i += 2)
			{
				// calculate angles
				delta_x = coordinates_2[i + 2] - coordinates_2[i];
				delta_y = coordinates_2[i + 3] - coordinates_2[i + 1];
				angle = round(atan2(delta_x, delta_y) * 180 / PI) + 360;

				// log angles
				log_draw_angles << angle - 360 << endl;

				// save angles to array
				draw_angles[num_draw_angles] = angle;
				num_draw_angles++;
			}

			//cout << "num_draw_angles = " << num_draw_angles << endl;

			// scale angle
			temp = num_draw_angles % 3;
			if (temp == 0)
			{
				num_data_byte = num_draw_angles / 3;
			}
			else if (temp == 1)
			{
				num_data_byte = (num_draw_angles - 1) / 3;
			}
			else
			{
				num_data_byte = (num_draw_angles - 2) / 3;
			}

			//cout << "num_data_byte = " << num_data_byte << endl;

			for (int i = 0; i < num_data_byte; i++)
			{
				draw_scale_angles[i + 1] = round((draw_angles[3 * i] + draw_angles[3 * i + 1] + draw_angles[3 * i + 2]) / 3);
				// log scale angles
				log_draw_scale_angles << draw_scale_angles[i + 1] - 360 << endl;
			}

			draw_scale_angles[0] = num_data_byte + 360;

			for (int i = 0; i <= num_data_byte; i++)
			{
				data_byte[2 * i] = draw_scale_angles[i] / 256;
				data_byte[2 * i + 1] = draw_scale_angles[i] % 256;

				// log 2 bytes
				log_serial_angles << int(data_byte[2 * i]) << "," << int(data_byte[2 * i + 1]) << "," << draw_scale_angles[i] - 360 << endl;
			}
			...
			if (hSerial != INVALID_HANDLE_VALUE)
			{
				...
				WriteFile(hSerial, data_byte, num_draw_angles * 2, &bytes_written, NULL);
	...
	log_draw_angles.close();
	log_draw_scale_angles.close();
	log_serial_angles.close();
...
void detect_hand(IplImage* src, IplImage* lower, IplImage* upper, IplImage* mask, IplImage* msk_bgsubtract, IplImage* msk_color, IplImage* msk_fg, IplImage* dst)
{
	...
	if (blob.m_nBlobs)
	{
		if (blob.m_recBlobs[Hand_Blob_Index].width>threshold_blob_width)		blob.m_recBlobs[Hand_Blob_Index].width = threshold_blob_width;
