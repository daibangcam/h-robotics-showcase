...
{
	IplImage* av = cvCreateImage(size_small, IPL_DEPTH_32F, 3);
	IplImage* sgm = cvCreateImage(size_small, IPL_DEPTH_32F, 3);
	...
	cvSub(av, sgm, lower);
	cvSubS(lower, cvScalarAll(ZETA), dst1);

	cvAdd(av, sgm, upper);
	cvAddS(upper, cvScalarAll(ZETA), dst2);
}

//////////////////// COMPARE ////////////////////
void Compare(IplImage* src1, IplImage* src2, IplImage* src3, IplImage* dst)
{
	IplImage* tmp = cvCreateImage(size_small, IPL_DEPTH_32F, 3);
	cvConvert(src1, tmp);
	// display different
	cvInRange(tmp, src2, src3, dst);				// cost much
	cvNot(dst, dst);
	cvErode(dst, dst, 0, 3);			// closing
	cvDilate(dst, dst, 0, 2);
}