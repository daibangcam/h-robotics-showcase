...
void main()
{
	...
	unsigned short int draw_coordinates[num];
	...
	// Log Draw Coordinates
	ofstream draw_angle;
	draw_angle.open("_log_draw_angle.csv");
	//draw_coordinate << "angle" << endl;

	// Log Serial Coordinates
	ofstream serial_coordinate;
	serial_coordinate.open("_log_serial_coordinates.csv");
	//serial_coordinate << "high byte" << "," << "low_byte" << endl;
	...
	while (1)
	{
		...
		if (hand_mode == 2)
		{
			...
			if (last_num_fingers == 2 && num_fingers == 3)
			{
				...
				side_1 = round(num_coordinates*0.1) * 2;
				...
				for (int i = side_1; i < side_2; i += 2)
				{
					...
					// log angles
					draw_coordinate << angle - 360 << endl;
				...
				// scale angle
				temp = num_draw_coordinates % 3;
				if (temp == 0)
				{
					num_data_byte = num_draw_coordinates / 3;
				}
				else if (temp == 1)
				{
					num_data_byte = (num_draw_coordinates - 1) / 3;
				}
				else
				{
					num_data_byte = (num_draw_coordinates - 2) / 3;
				}

				//cout << num_data_byte << endl;

				for (int i = 0; i < num_data_byte; i++)
				{
					draw_angles[i + 1] = round((draw_coordinates[3 * i] + draw_coordinates[3 * i + 1] + draw_coordinates[3 * i + 2]) / 3);
					// log scale angles
					draw_angle << draw_angles[i + 1] - 360 << endl;
				}

				draw_angles[0] = num_data_byte + 360;

				for (int i = 0; i <= num_data_byte; i++)
				{
					data_byte[2 * i] = draw_angles[i] / 256;
					data_byte[2 * i + 1] = draw_angles[i] % 256;

					// log 2 bytes
					serial_coordinate << int(data_byte[2 * i]) << "," << int(data_byte[2 * i + 1]) << endl;
				}
				...
				if (hSerial != INVALID_HANDLE_VALUE)
				{
					...
					WriteFile(hSerial, data_byte, num_draw_coordinates * 2, &bytes_written, NULL);
					WriteFile(hSerial, end_byte, 1, &e_bytes_written, NULL);
				}

				// reset array
				num_coordinates = 0;
				num_draw_coordinates = 1;
			}
